...
def student_train(data):
    ...
    gt_prob = np.eye(args.num_classes)[vaild_labels]
    ...
    controller = Controller(input_dim=args.dim_option_6)        # MLP fusion ratio
    optimizer_student = torch.optim.Adam(student.parameters(), lr=0.1)
    optimizer_controller = torch.optim.Adam(controller.parameters(), lr=0.1, weight_decay=0.5)
    valid_loss_list = []
    ...
    for ep in range(args.num_epochs_stu):
        ...
        kd_lower_loss = kl_loss(student_knowledge_soft, teacher_knowledge_soft).sum(1)      # knowledge distillation loss
        gt_lower_loss = loss_func(student_pred_prob, torch.from_numpy(train_labels))     # ground truth loss
        lower_loss = 0.5 * args.temperature ** 2 * kd_lower_loss + 0.5 * gt_lower_loss   # total loss
        ...
        with torch.no_grad():
            student_knowledge, student_pred_prob = student(torch.from_numpy(valid_features).float())
            student_knowledge_soft = nn.functional.log_softmax(student_knowledge / args.temperature, dim=1)
            teacher_knowledge, teacher_pred_prob = teacher(torch.from_numpy(valid_features).float())
            teacher_knowledge_soft = nn.functional.softmax(teacher_knowledge / args.temperature, dim=1)
        mlp_state = state_design(student_pred_prob, teacher_pred_prob, gt_prob)    # mlp state design
        alpha = torch.squeeze(controller(mlp_state.float()))     # learned fusion ratio
        kd_upper_loss = kl_loss(student_knowledge_soft, teacher_knowledge_soft).sum(1)   # knowledge distillation loss
        gt_upper_loss = loss_func(student_pred_prob, torch.from_numpy(valid_labels))   # ground truth loss
        upper_loss = alpha * args.temperature ** 2 * kd_upper_loss + (1 - alpha) * gt_upper_loss     # total loss
        upper_loss = torch.mean(upper_loss)
        upper_loss.backward()
        optimizer_controller.step()
        print("-----------------------------------------------------------------------")
        print("The " + str(ep) + "-th epoch upper level")
        print("-----------------------------------------------------------------------")
        print("kd loss: " + str(torch.mean(kd_upper_loss)))
        print("gt loss: " + str(torch.mean(gt_upper_loss)))
    ...
    torch.save(student, "../result/bilevel/control_kd_student_option_6.pkl")  # save the student network
...
def student_test(data):
    ...
    student = torch.load("../result/bilevel/control_kd_student_option_6.pkl")     # load student network
