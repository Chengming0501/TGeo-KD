...
def parse_args():
    parser = ArgumentParser(description="anl_kd")
...
def student_train(data):
    ...
    for ep in range(args.num_epochs_stu):
        ...
        kd_loss = kl_loss(student_knowledge_soft, teacher_knowledge_soft)   # kd loss
        ...
        if alpha % 500 == 0:    # gradually decrease the ratio from 1 to 0
            alpha -= 0.1
    ...
    torch.save(student, "../result/baseline_anl_kd_student.pkl")  # save the student network
...
def student_test(data):
    ...
    student = torch.load("../result/baseline_anl_kd_student.pkl")     # load student network
