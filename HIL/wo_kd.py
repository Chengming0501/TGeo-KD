...
def parse_args():
    parser = ArgumentParser(description="wo_kd")
...
def student_wo_KD_train(data):
    ...
    for ep in range(args.num_epochs_stu):
        student.train()
        _, train_pred = student(torch.from_numpy(train_features).float())
        loss = loss_func(train_pred, torch.from_numpy(train_labels))
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    torch.save(student, "../result/wo_kd_student.pkl")     # save the student network
...
def student_wo_kd_test(data):
    ...
    student = torch.load("../result/wo_kd_student.pkl")  # load student network
...
if __name__ == '__main__':
    ...
    # without KD: train and test student network
    # student = student_wo_KD_train(dataset)
    student_wo_kd_test(dataset)