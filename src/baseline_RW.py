...
def parse_args():
    parser = ArgumentParser(description="rw_kd")
...
def student_train(data):
    ...
    alpha = torch.nn.Parameter(torch.ones(m) / 2, requires_grad=True)
    optimizer_student = torch.optim.Adam(student.parameters(), lr=0.01)
    optimizer_controller = torch.optim.Adam(params=[alpha], lr=0.01)
    valid_loss_list = []
    ...
    for ep in range(args.num_epochs_stu):
        ...
        gt_loss = loss_func(student_pred_prob, torch.from_numpy(train_labels))  # ce loss in student
    ...
    torch.save(student, "../result/baseline_rw_kd_student.pkl")  # save the student network
...
def student_test(data):
    ...
    student = torch.load("../result/baseline_rw_kd_student.pkl")     # load student network
