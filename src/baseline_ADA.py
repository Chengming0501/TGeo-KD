...
def parse_args():
    parser = ArgumentParser(description="ada_kd")
    ...
    parser.add_argument('--num_epochs_stu', type=int, default=2000, help="number of epoch for training student")
...
def student_train(data):
    ...
    alpha = np.zeros(m)
    _, teacher_pred_prob = teacher(torch.from_numpy(train_features).float())
    for i in range(args.num_classes):
        sample_index = np.where(gt_prob[:, i] == 1)
        teacher_pred_prob_class = torch.squeeze(teacher_pred_prob[sample_index, :])
        alpha[sample_index] = teacher_pred_prob_class[:, i].detach().numpy() - \
                              np.sum(np.delete(teacher_pred_prob_class.detach().numpy(), obj=i, axis=1), axis=1) / (args.num_classes - 1)
    alpha = torch.from_numpy(np.where(alpha < 0, 0, alpha)).float()
    ...
    for ep in range(args.num_epochs_stu):
        ...
        gt_loss = loss_func(student_pred_prob, torch.from_numpy(train_labels))  # gt loss
        loss = alpha * kd_loss + (1 - alpha) * gt_loss     # total loss
    ...
    torch.save(student, "../result/baseline_ada_kd_student.pkl")  # save the student network
...
def student_test(data):
    ...
    student = torch.load("../result/baseline_ada_kd_student.pkl")     # load student network
